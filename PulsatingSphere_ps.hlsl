...
float4 main(WiggleShaderInput input) : SV_Target
{
    // Calculate scrolled texture coordinates
    float2 scrolledUVs = input.uv + (0.01f * wiggle);
    ...
    float3 diffuseMapColour = DiffuseMap.Sample(TexSampler, scrolledUVs).rgb;
