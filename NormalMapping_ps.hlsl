...
float4 main(NormalMappingPixelShaderInput input) : SV_Target
{
   ...
   // Lighting equations
    ...
    // Apply color variation - only to the red component of the texture color
    // This directly modifies the 'r' component while leaving 'g' and 'b' as they are in the original texture color
    float4 modifiedTextureColour = textureColour;
    modifiedTextureColour.rgb = textureColour.rgb * colorVariation.rgb; // Adjust only the 'r' component

    // Combine the lighting calculations with the modified texture color
    // Note: Ensure the lighting calculations result in a value that makes sense when combined with your texture color
    float3 finalColour = (gAmbientColour + diffuseLight1 + diffuseLight3) * modifiedTextureColour.rgb +
                     (specularLight1 + specularLight3) * textureColour.a; // Using original alpha

    // Output the final color
    return float4(finalColour, 1.0f); // Using 1.0f for alpha for simplicity
}