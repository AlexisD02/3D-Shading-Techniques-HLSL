...
ID3D11BlendState* gMultiplicativeBlendState = nullptr;
ID3D11BlendState* gAlphaBlendState = nullptr;
...
bool CreateStates()
{
    ...
    // Set up blending factors and operation for multiplicative blending
    D3D11_BLEND_DESC multiplicativeBlendDesc = {};
    multiplicativeBlendDesc.RenderTarget[0].BlendEnable = TRUE;
    multiplicativeBlendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_DEST_COLOR;  // Multiply by destination color
    multiplicativeBlendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ZERO;      // Ignore source color
    multiplicativeBlendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;      // Add the results

    // Set other blending options to default values
    multiplicativeBlendDesc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;
    multiplicativeBlendDesc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;
    multiplicativeBlendDesc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
    multiplicativeBlendDesc.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;

    // Create the blending state object
    if (FAILED(gD3DDevice->CreateBlendState(&multiplicativeBlendDesc, &gMultiplicativeBlendState)))
    {
    ...
    // Create the blending state object
    if (FAILED(gD3DDevice->CreateBlendState(&alphaBlendDesc, &gAlphaBlendState)))
    {
        // Handle error
    }
